...
+(void)clearStatus
{
    ...
    [Tool removeObjectForKey:JIsTieCard];
    [Tool removeObjectForKey:JIsSetSecretShoushi];
    [Tool removeObjectForKey:JSaveSecretShoushi];
    [Tool removeObjectForKey:JIsNamesure];
}

#pragma mark 1、NSuserDefault存值
+ (void)setObject:(id)object forKey:(NSString *)key{
    [JUserDefaults setObject:object forKey:key];
    [JUserDefaults synchronize];
}
+ (void)setBool:(BOOL)b forKey:(NSString *)key{
    [JUserDefaults setBool:b forKey:key];
    [JUserDefaults synchronize]; 
}
+ (void)removeObjectForKey:(NSString *)key{
    [JUserDefaults removeObjectForKey:key];
    [JUserDefaults synchronize];
}
#pragma mark 2、NSuserDefault取值
+ (id)objectForKey:(NSString *)key{
    return [JUserDefaults objectForKey:key];
}
+ (BOOL)boolForKey:(NSString *)key{
    return [JUserDefaults boolForKey:key];
}
    ...
    [manger setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusNotReachable://没有网络
                [Tool setBool:NO forKey:isNetWorking];
                break;
            case AFNetworkReachabilityStatusUnknown://未知网络
                [Tool setBool:YES forKey:isNetWorking];
                break;
