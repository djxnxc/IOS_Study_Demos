...
static const CGFloat CSToastDefaultDuration     = 1.6;
...
-(void)makeToast:(NSString *)message ifSucess:(BOOL)ifSucess
{
    UIView *toast = [self viewForMessage:message ifSucess:ifSucess];
    [self showToast:toast duration:CSToastDefaultDuration position:CSToastActivityDefaultPosition];
    
}
- (UIView *)viewForMessage:(NSString *)message ifSucess:(BOOL)ifSucess {
    ...
    wrapperView.backgroundColor = [[UIColor colorWithRed:130/255.0f green:130/255.0f blue:130/255.0f alpha:1] colorWithAlphaComponent:CSToastOpacity];
    if (CSToastDisplayShadow) {
        wrapperView.layer.shadowColor = [UIColor lightGrayColor].CGColor;
...
#warning 显示的警告的背景色
    ...
    {
        messageLabel.textColor=[UIColor whiteColor];
        wrapperView.backgroundColor = [[UIColor lightGrayColor] colorWithAlphaComponent:CSToastOpacity];
...
- (void)makeToast:(NSString *)message duration:(CGFloat)interval{
    [self makeToast:message duration:interval position:@"center"];
}
