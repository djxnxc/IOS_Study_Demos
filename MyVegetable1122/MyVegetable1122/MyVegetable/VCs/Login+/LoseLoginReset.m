...
#pragma mark 确认修改
- (void)nextNetWorking {
    if (([self.mima.text length]<6) || ([self.mima.text length]>20))
    {
        [self.view makeToast:@"密码应为6-20字符!" ifSucess:NO];
        return;
    }
    
    if (![self.mima.text isEqualToString:self.mimaa.text]) {
        [self.view makeToast:@"两次输入密码不一致！" duration:1.5];
        return;
    }
    ...
    NSDictionary *dicParams =[NSDictionary dictionaryWithObjectsAndKeys:
                              ...
                              ,[NSString MD5:self.mimaa.text],@"comfirmPwd",
                              self.code,@"code",
...
- (void)transactionFinished:(HZMResponse *)response {
    ...
    if ([response.requestId isEqualToString:LoginCZMM_NetWoring]) {//重置密码
        if([response.responseCodeOriginal isEqualToString:@"1"]){
            ...
            //[self.navigationController popToViewController:ll animated:YES];
